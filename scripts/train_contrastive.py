...
def main(args):
    try:
        ...
        logger.info("Starting CONTRASTIVE LEARNING Experiment (Phase 28)...")
        log_memory("Start")
        ...
        logger.info("=== STEP 1: SupCon Feature Learning ===")
        model = trainer.train_contrastive(train_loader, epochs=args.supcon_epochs)
        
        # Step 2: Linear Probing
        logger.info("=== STEP 2: Linear Probing (Classification) ===")
        trainer.train_linear_probe(train_loader, val_loader, epochs=args.probe_epochs)
        
        # Verify result
        final_loss = trainer.validate(val_loader)
        logger.info(f"✅ Final Validation Loss (Contrastive Agent): {final_loss:.4f}")
        
        # Export
        out_path = config.MODELS_DIR / "model_contrastive.pt"
